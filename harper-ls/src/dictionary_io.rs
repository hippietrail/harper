...
pub async fn load_dict(path: impl AsRef<Path>, langiso639: &str) -> Result<MutableDictionary> {
    let file = File::open(path.as_ref()).await?;
    let read = BufReader::new(file);

    dict_from_word_list(read, langiso639).await
}
...
mod tests {
    ...
    fn get_test_unsorted_dict() -> MutableDictionary {
        let mut test_unsorted_dict = MutableDictionary::new("en");
