...
fn daily_basis() {
    assert_suggestion_result(
        "Some of the observations will look naive and obvious for the people who work with Zig on daily basis",
        test_linter(),
        "Some of the observations will look naive and obvious for the people who work with Zig on a daily basis",
    )
}
...
fn yearly_basis() {
    assert_suggestion_result(
        ...
        test_linter(),
        "Gives report about the different type of product ordered on a yearly basis and provided yearly comparsions about the profits and the loss.",
    )
}
...
fn weekly_basis() {
    assert_suggestion_result(
        ...
        test_linter(),
        "On a weekly basis, the models are retrained, plus the monitoring routines are performed as usual.",
    )
}
...
fn monthly_basis() {
    assert_suggestion_result(
        "I'm able to extract this information by generating Category Reports on monthly basis in separate browser tabs and comparing them \"manually\"",
        test_linter(),
        "I'm able to extract this information by generating Category Reports on a monthly basis in separate browser tabs and comparing them \"manually\"",
    )
}
...
fn hourly_basis() {
    assert_suggestion_result(
        ...
        test_linter(),
        "A Big Query SQL request to monitor and explain GA4 metrics on an hourly basis.",
    )
}

#[test]
fn bi_hyphen_weekly_basis() {
    assert_suggestion_result(
        "Patch release is released on bi-weekly basis.",
        test_linter(),
        "Patch release is released on a bi-weekly basis.",
    )
}
...
fn biweekly_basis() {
    assert_suggestion_result(
        "Topics tracked in the repo are reviewed on biweekly basis in the Compound Developer Community Calls in Discord.",
        test_linter(),
        "Topics tracked in the repo are reviewed on a biweekly basis in the Compound Developer Community Calls in Discord.",
    )
}
...
fn annual_basis() {
    assert_suggestion_result(
        ...
        test_linter(),
        "Schedule the campain for two weeks and to automatically repeat on an annual basis.",
    )
}
...
fn bimonthly_basis() {
    assert_suggestion_result(
        "Calculates tutor payouts and donations on bimonthly basis, proportional to hours logged",
        test_linter(),
        "Calculates tutor payouts and donations on a bimonthly basis, proportional to hours logged",
    )
}
...
fn biannual_basis() {
    assert_suggestion_result(
        "Although I use to go for the must outings on biannual basis to relax myself getting fresh airs full of refreshes",
        test_linter(),
        "Although I use to go for the must outings on a biannual basis to relax myself getting fresh airs full of refreshes",
    );
}

#[test]
fn bi_hyphen_monthly_basis() {
    assert_suggestion_result(
        "you can do transfer between your brokerage accounts ( taxable ) in Fidelity as a bill payment on bi-monthly basis",
        test_linter(),
        "you can do transfer between your brokerage accounts ( taxable ) in Fidelity as a bill payment on a bi-monthly basis",
    );
}

#[test]
fn bi_space_weekly_basis() {
    assert_suggestion_result(
        "I got ai india head ( products)  from a top tier company coaching me since last December on bi weekly basis.",
        test_linter(),
        "I got ai india head ( products)  from a top tier company coaching me since last December on a bi weekly basis.",
    )
}

#[test]
fn bi_space_monthly_basis() {
    assert_suggestion_result(
        "After you get the class under control this way, you can start calling home on bi monthly basis.",
        test_linter(),
        "After you get the class under control this way, you can start calling home on a bi monthly basis.",
    )
}

#[test]
fn bi_space_annual_basis() {
    assert_suggestion_result(
        "Surprise surprise, its the same with the German consulate and the US consulate, which I deal with on bi annual basis along with 23 other national consular officials.",
        test_linter(),
        "Surprise surprise, its the same with the German consulate and the US consulate, which I deal with on bi annual basis along with 23 other national consular officials.",
    )
}
...
#[test]
fn bi_hyphen_annual_basis() {
    assert_suggestion_result(
        "I will tell her to do this on bi-annual basis just to keep it working.",
        test_linter(),
        "I will tell her to do this on bi-annual basis just to keep it working.",
    )
}
