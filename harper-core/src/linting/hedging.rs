...
impl Default for Hedging {
    fn default() -> Self {
        ...
        let patterns: Vec<Box<dyn Pattern>> = phrases
            .into_iter()
            .map(|s| Box::new(ExactPhrase::from_phrase(s, "en")) as Box<dyn Pattern>)
            .collect();
...
mod tests {
    ...
    fn does_not_flag_clean_text() {
        assert_lint_count("This is clear and direct.", "en", Hedging::default(), 0);
    }
    ...
    fn phrase_with_trailing_comma() {
        ...
        assert_lint_count(text, "en", Hedging::default(), 1);
    }
