use crate::{CharStringExt, char_ext::CharExt};

/// Handles regular English noun singular<->plural conversion patterns
pub struct RegularNouns;
...
impl RegularNouns {
    ...
    pub fn get_plurals(singular: &[char], dict: &impl crate::spell::Dictionary) -> Vec<Vec<char>> {
        // Filter candidates by dictionary validation
        Self::get_plural_candidates(singular)
            .into_iter()
            .filter(|candidate| {
    ...
    pub fn get_singulars(plural: &[char], dict: &impl crate::spell::Dictionary) -> Vec<Vec<char>> {
        // Filter candidates by dictionary validation
        Self::get_singular_candidates(plural)
            .into_iter()
            .filter(|candidate| {
                dict.get_word_metadata(candidate)
...
mod tests {
    ...
    fn test_regular_plurals_add_s() {
        ...
        let plurals = RegularNouns::get_plurals(&['c', 'a', 't'], &dict);
    ...
    fn test_regular_plurals_y_to_ies() {
        let dict = FstDictionary::curated();
        let plurals = RegularNouns::get_plurals(&['c', 'i', 't', 'y'], &dict);
    ...
    fn test_regular_plurals_vowel_y_adds_s() {
        let dict = FstDictionary::curated();
        let plurals = RegularNouns::get_plurals(&['b', 'o', 'y'], &dict);
    ...
    //     let dict = FstDictionary::curated();
    //     let plurals = RegularNouns::get_plurals(&['w', 'i', 'f', 'e'], &dict);
    ...
    fn test_regular_plurals_f_to_ves() {
        let dict = FstDictionary::curated();
        let plurals = RegularNouns::get_plurals(&['w', 'o', 'l', 'f'], &dict);
    ...
    fn test_regular_plurals_es_endings() {
        let dict = FstDictionary::curated();
        let plurals = RegularNouns::get_plurals(&['b', 'u', 's'], &dict);
    ...
    fn test_regular_plurals_o_to_oes() {
        let dict = FstDictionary::curated();
        let plurals = RegularNouns::get_plurals(&['p', 'o', 't', 'a', 't', 'o'], &dict);
    ...
    fn test_regular_plurals_vowel_o_adds_s() {
        let dict = FstDictionary::curated();
        let plurals = RegularNouns::get_plurals(&['r', 'a', 'd', 'i', 'o'], &dict);
    ...
    fn test_single_letter_words() {
        ...
        let plurals = RegularNouns::get_plurals(&['a'], &dict);
        assert!(plurals.is_empty());

        let singulars = RegularNouns::get_singulars(&['s'], &dict);
    ...
    fn test_empty_string() {
        let dict = FstDictionary::curated();
        let plurals = RegularNouns::get_plurals(&[], &dict);
        assert!(plurals.is_empty());

        let singulars = RegularNouns::get_singulars(&[], &dict);
        assert!(singulars.is_empty());
    }
    ...
    fn test_chars_versions() {
        ...
        let singulars = RegularNouns::get_singulars(&['c', 'i', 't', 'i', 'e', 's'], &dict);
    ...
    fn test_singular_from_plural_remove_s() {
        let dict = FstDictionary::curated();
        let singulars = RegularNouns::get_singulars(&['c', 'a', 't', 's'], &dict);
    ...
    fn test_singular_from_plural_ves_to_fe() {
        let dict = FstDictionary::curated();
        let singulars = RegularNouns::get_singulars(&['w', 'i', 'v', 'e', 's'], &dict);
    ...
    fn test_singular_from_plural_es_removal() {
        ...
        let singulars =
            RegularNouns::get_singulars(&['c', 'l', 'a', 's', 's', 'e', 's'], &dict);
    ...
    fn test_singular_from_plural_oes_to_o() {
        let dict = FstDictionary::curated();
        let singulars =
            RegularNouns::get_singulars(&['p', 'o', 't', 'a', 't', 'o', 'e', 's'], &dict);
    ...
    fn test_case_insensitive_plurals() {
        ...
        let plurals = RegularNouns::get_plurals(&lowercase_city, &dict);
        assert!(plurals.contains(&['c', 'i', 't', 'i', 'e', 's'].to_vec()));

        // Mixed case input: stem case preserved, suffix lowercase
        let mixed_city: Vec<char> = "City".chars().collect();
        let plurals = RegularNouns::get_plurals(&mixed_city, &dict);
        assert!(plurals.contains(&['C', 'i', 't', 'i', 'e', 's'].to_vec()));
    }
    ...
    fn test_case_insensitive_singulars() {
        ...
        let singulars = RegularNouns::get_singulars(&lowercase_cities, &dict);
        assert!(singulars.contains(&['c', 'i', 't', 'y'].to_vec()));

        // Mixed case input: case preserved from stem
        let mixed_cities: Vec<char> = "Cities".chars().collect();
        let singulars = RegularNouns::get_singulars(&mixed_cities, &dict);
        assert!(singulars.contains(&['C', 'i', 't', 'y'].to_vec()));
    }
    ...
    fn test_generated_candidates_must_validate() {
        let dict = FstDictionary::curated();
        let plurals = RegularNouns::get_plurals(&['a', 'n', 't'], &dict);
